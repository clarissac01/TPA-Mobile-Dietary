...
class DinnerFragment(var currDay: Int) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        initMenu()
        view.findViewById<ConstraintLayout>(R.id.dinnerLayout).setOnClickListener{
            val intent = Intent(
                context,
                ...
                db.collection("Journey").whereEqualTo("userID", userId).whereEqualTo("Date", Timestamp.now()).get().addOnSuccessListener {
                    if(!it?.isEmpty!!){
                        if(it.documents.first().get("dinnerMenu")!= null){
                            ...
                            view.findViewById<Button>(R.id.floatingActionButton4).visibility = View.INVISIBLE
                        }
                    }else{
                        view.findViewById<Button>(R.id.changeDinner).visibility = View.VISIBLE
                        view.findViewById<Button>(R.id.floatingActionButton4).visibility = View.VISIBLE
                    }
                }
        ...
        view.findViewById<Button>(R.id.changeDinner).setOnClickListener {
            val intent = Intent(
                context,
                Meal::class.java
            )
    ...
    fun initMenu(){
        ...
        db.collection("users").whereEqualTo("username", user.displayName).addSnapshotListener() { it, _ ->
            if(!it?.isEmpty!!){
                ...
                menuId = getMapping["dinnerMenu"].toString()
